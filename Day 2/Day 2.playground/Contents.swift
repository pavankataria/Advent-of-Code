...
typealias Foot = Int
typealias SquareFoot = Int
typealias CubeFoot = Int

protocol WrappingProtocol {
    func totalWrappingPaperRequired() -> SquareFoot
    func totalWrappingRibonRequired() -> Foot
}
...
struct Present {
    ...
    let bottomArea: SquareFoot
    let frontArea: SquareFoot
    let sideArea: SquareFoot
    
    var surfaceArea: SquareFoot {
...
extension Present : WrappingProtocol {
    func totalWrappingPaperRequired() -> SquareFoot {
        let smallestSurfaceArea = min(bottomArea, frontArea, sideArea)
        return surfaceArea * 2 + smallestSurfaceArea
    }
    func totalWrappingRibonRequired() -> Foot {
        let smallestEdge = [length, width, height].sort(<)[0]
        let smallEdge = [length, width, height].sort(<)[1]
        return smallestEdge * 2 + smallEdge * 2 + volume
    }
}
...
var totalWrappingRibon = 0
for present in presents {
    totalWrappingPaper += present.totalWrappingPaperRequired()
    totalWrappingRibon += present.totalWrappingRibonRequired()
}
...
print("Total wrapping ribon required: \(totalWrappingRibon)")